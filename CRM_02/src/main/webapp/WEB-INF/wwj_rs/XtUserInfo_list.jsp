...
<div class="page-container">
	...
	<a href="javascript:;" onclick="admin_add('添加管理员','xtuserctrl/goadduser.do','800','450')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a></span> <span class="r">共有数据：<strong></strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="19">员工信息表</th>
			</tr>
			<tr class="text-c">
		...
		<tbody>
		   <c:forEach items="${p.list }" var="XTUU">
			<tr class="text-c">
				<td><input type="checkbox" value="${XTUU.workerId }" name=""></td>
				<td>${XTUU.workerId }</td>
				<td>${XTUU.workerName }</td>
				<td>${XTUU.workerSex}</td>
				<td>${XTUU.workerPhoto}</td>
				<td>${XTUU.idCard}</td>
				<td>${XTUU.homeAdd}</td>
				<td>${XTUU.nowAdd}</td>
				<td>${XTUU.workerEdu}</td>
				<td>${XTUU.politics}</td>
				<td>${XTUU.school}</td>
				...
				<td>
				<fmt:formatDate value="${XTUU.lastModifyDate}"
